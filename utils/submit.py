...
from lib.detector.common.utils import to_unicode
    ...
    if args.url:
        ...
        if args.remote:
            if not HAVE_REQUESTS:
                print("Error: you need to install python-requests (`pip install requests`)")
                return False
            ...
            try:
                ...
                print("Error: unable to send URL: {0}".format(e))
                return False
        ...
        if task_id:
            if not args.quiet:
                print(u"Success : URL \"{0}\" added as task with ID {1}".format(target, task_id))
        else:
        ...
        if task_id:
            if not args.quiet:
                print(u"Success: Baseline analysis added as task with ID {0}".format(task_id))
        else:
            print("Error: adding task to database")
    else:
        target = to_unicode(args.target)
        ...
        if not os.path.exists(path):
            print(u"Error: the specified file/folder does not exist at path \"{0}\"".format(path))
            return False
        ...
        for file_path in files:
            if not File(file_path).get_size():
                if not args.quiet:
                    print("Empty: sample {0} (skipping file)".format(file_path))
            ...
            if args.remote:
                ...
                try:
                    ...
                    print("Error: unable to send file: {0}".format(e))
                    return False
            ...
            if task_id:
                if not args.quiet:
                    print(u"Success : File \"{0}\" added as task with ID {1}".format(file_path, task_id))
            else:
                print("Error : adding task to database")
