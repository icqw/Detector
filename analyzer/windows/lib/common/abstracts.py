...
from lib.common.exceptions import DetectorPackageError
...
class Package(object):
    ...
    def get_path(self, application):
        ...
        raise DetectorPackageError("Unable to find any %s executable." %
                                 application)
        ...
        for rootkey, subkey, values in regkeys:
            ...
            for key, value in values.items():
                if isinstance(value, str):
                    ...
                    raise DetectorPackageError("Invalid value type: %r" % value)
    ...
    def execute(self, path, args, mode=None, maximize=False):
        ...
        if not p.execute(path=path, args=args, dll=dll, free=free,
                         ...
                         maximize=maximize):
            raise DetectorPackageError("Unable to execute the initial process, "
                                     "analysis aborted.")
