...
from lib.detector.common.exceptions import DetectorMachineError
...
class Machinery(object):
    ...
    def pcap_path(self, task_id):
        ...
        return os.path.join(DETECTOR_ROOT, "storage", "analyses",
                            "%s" % task_id, "dump.pcap")
    ...
    def _initialize(self, module_name):
        ...
        for machine_id in mmanager_opts["machines"].strip().split(","):
            ...
            except (AttributeError, DetectorOperationalError) as e:
    ...
    def _initialize_check(self):
        ...
        @note: in machine manager modules you may override or superclass
               his method.
        @raise DetectorMachineError: if a misconfiguration or a unkown vm state
                                   is found.
        """
        try:
        ...
        for machine in self.machines():
            ...
            try:
                ...
                raise DetectorCriticalError(msg)

        if not self.options_globals.timeouts.vm_state:
            raise DetectorCriticalError("Virtual machine state change timeout "
    ...
    def shutdown(self):
        ...
        @raise DetectorMachineError: if unable to stop machine.
        """
    ...
    def _wait_status(self, label, state):
        ...
        while current not in state:
            log.debug("Waiting %i detectorseconds for machine %s to switch "
...
class LibVirtMachinery(Machinery):
    ...
    def __init__(self):
        if not HAVE_LIBVIRT:
            raise DetectorDependencyError("Unable to import libvirt")
...
class Signature(object):
    """Base class for Detector signatures."""
    name = ""
    description = ""
    severity = 1
    order = 1
