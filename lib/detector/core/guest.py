...
from lib.detector.common.constants import DETECTOR_GUEST_PORT, DETECTOR_GUEST_INIT
from lib.detector.common.constants import DETECTOR_GUEST_COMPLETED
from lib.detector.common.constants import DETECTOR_GUEST_FAILED
from lib.detector.common.exceptions import DetectorGuestError
from lib.detector.common.utils import TimeoutServer
...
class OldGuestManager(object):
    ...
    def wait(self, status):
        ...
        while db.guest_get_status(self.task_id) == "starting":
            ...
            if time.time() > end:
                raise DetectorGuestError("{0}: the guest initialization hit the "
    ...
    def upload_analyzer(self, monitor):
        ...
        root = os.path.join(DETECTOR_ROOT, "analyzer", self.platform)
        ...
        if self.platform == "windows":
            dirpath = os.path.join(DETECTOR_ROOT, "data", "monitor", monitor)
        ...
        try:
            ...
            raise DetectorGuestError("{0}: guest communication timeout: unable "
    ...
    def start_analysis(self, options, monitor):
        ...
        try:
            ...
            try:
                ...
                raise DetectorGuestError("{0}: unable to upload config to "
            ...
            if options["category"] == "file":
                try:
                    ...
                    raise DetectorGuestError("Unable to read {0}, error: "
                ...
                try:
                    ...
                    raise DetectorGuestError("{0}: unable to upload malware to "
        ...
        # exception and abort the analysis.
            ...
            raise DetectorGuestError("{0}: guest communication timeout, check "
    ...
    def wait_for_completion(self):
        ...
        while db.guest_get_status(self.task_id) == "running":
            ...
            if time.time() > end:
                raise DetectorGuestError("The analysis hit the critical timeout, terminating.")
            ...
            if status == DETECTOR_GUEST_COMPLETED:
                log.info("%s: analysis completed successfully", self.id)
                break
            elif status == DETECTOR_GUEST_FAILED:
                error = self.server.get_error()
                if not error:
                    error = "unknown error"

                raise DetectorGuestError("Analysis failed: {0}".format(error))
            else:
...
class GuestManager(object):
    ...
    Detector Agent which features a more abstract but more feature-rich API."""
    ...
    def start_analysis(self, options, monitor):
        ...
        if r.status_code == 501:
            # log.info("Detector 2.0 features a new Agent which is more "
        ...
        log.info("Guest is running Detector Agent %s (id=%s, ip=%s)",
