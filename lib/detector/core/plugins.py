...
from lib.detector.common.abstracts import Auxiliary, Machinery, LibVirtMachinery, Processing
from lib.detector.common.abstracts import Report, Signature
from lib.detector.common.config import Config, parse_options
...
def import_plugin(name):
    try:
        ...
        raise DetectorCriticalError("Unable to import plugin "
                                  "\"{0}\": {1}".format(name, e))
    else:
        load_plugins(module)
...
class RunProcessing(object):
    ...
    def __init__(self, task):
        ...
        self.task = task
        self.analysis_path = os.path.join(DETECTOR_ROOT, "storage", "analyses", str(task["id"]))
...
class RunSignatures(object):
    ...
    def __init__(self, results):
        ...
        # strip that part off.
        self.version = DETECTOR_VERSION.split("-")[0]
    ...
    def check_signature_version(self, signature):
        ...
        @return: check result.
        """
        # Check the minimum Detector version for this signature, if provided.
        if signature.minimum:
            try:
                # If the running Detector is older than the required minimum
                ...
                if StrictVersion(self.version) < StrictVersion(signature.minimum):
                    log.debug("You are running an older incompatible version "
                              "of Detector, the signature \"%s\" requires "
                              "minimum version %s.",
                              signature.name, signature.minimum)
                    return False

                if StrictVersion("1.2") > StrictVersion(signature.minimum):
                    log.warn("Detector signature style has been redesigned in "
                             "detector 1.2. This signature is not "
                             "compatible: %s.", signature.name)
                    return False

                if StrictVersion("2.0") > StrictVersion(signature.minimum):
                    log.warn("Detector version 2.0 features a lot of changes that "
        ...
        # Check the maximum version of Detector for this signature, if provided.
        if signature.maximum:
            try:
                # If the running Detector is newer than the required maximum
                # version, skip this signature.
