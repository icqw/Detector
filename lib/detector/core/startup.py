...
from lib.detector.common.colors import red, green, yellow, cyan
from lib.detector.common.config import Config
from lib.detector.common.constants import DETECTOR_ROOT, DETECTOR_VERSION
from lib.detector.common.exceptions import DetectorStartupError, DetectorDatabaseError
from lib.detector.common.exceptions import DetectorOperationalError
from lib.detector.common.utils import create_folders
from lib.detector.core.database import Database, TASK_RUNNING
from lib.detector.core.database import TASK_FAILED_ANALYSIS, TASK_PENDING
from lib.detector.core.plugins import import_plugin, import_package, list_plugins
from lib.detector.core.rooter import rooter, vpns
...
def check_python_version():
    """Checks if Python version is supported by Detector.
    @raise DetectorStartupError: if version is not supported.
    """
    if sys.version_info[:2] != (2, 7):
        raise DetectorStartupError("You are running an incompatible version "
...
def check_version():
    """Checks version of Detector."""
    cfg = Config()

    if not cfg.detector.version_check:
        return

    print(" Checking for updates...")

    url = "http://api.detectorsandbox.org/checkversion.php"
    data = urllib.urlencode({"version": DETECTOR_VERSION})
    ...
    if not r["error"]:
        if r["response"] == "NEW_VERSION" and r["current"] != "1.2":
            msg = "Detector Sandbox version %s is available now." % r["current"]
...
def init_logging():
    ...
    fh = logging.handlers.WatchedFileHandler(os.path.join(DETECTOR_ROOT, "log", "detector.log"))
...
def delete_file(*rel_path):
    filepath = os.path.join(DETECTOR_ROOT, *rel_path)
    ...
    try:
        ...
        log.warning(
            ...
            "your Detector setup to the latest version: %s.",
...
def init_modules(machinery=True):
    ...
    if machinery:
        import_plugin("modules.machinery." + Config().detector.machinery)
...
def init_yara():
    ...
    yara_root = os.path.join(DETECTOR_ROOT, "data", "yara")
...
def init_binaries():
    ...
    if not os.path.exists(monitor):
        raise DetectorStartupError(
            ...
            "Detector or when you've updated your Detector version by pulling "
...
def init_rooter():
    ...
    if not Config("vpn").vpn.enabled and detector.routing.route == "none":
        return
    ...
    try:
        s.connect(detector.detector.rooter)
        ...
        if e.strerror == "No such file or directory":
            raise DetectorStartupError(
                ...
                "and internet to none in detector.conf and enabled to no in "
                "vpn.conf)."
            )
        ...
        raise DetectorStartupError("Unknown rooter error: %s" % e)
...
def init_routing():
    ...
    if detector.routing.route not in ("none", "internet"):
        ...
        if detector.routing.route not in vpns:
            raise DetectorStartupError(
    ...
    if detector.routing.internet != "none":
        if not rooter("nic_available", detector.routing.internet):
            raise DetectorStartupError(
        ...
        rooter("enable_nat", detector.routing.internet)

def detector_clean():
    """Clean up detector setup.
    ...
    except DetectorDatabaseError as e:
    ...
    # Paths to clean.
    paths = [
        os.path.join(DETECTOR_ROOT, "db"),
        os.path.join(DETECTOR_ROOT, "log"),
        os.path.join(DETECTOR_ROOT, "storage"),
    ]
    ...
    for dirpath, dirnames, filenames in os.walk(DETECTOR_ROOT):
