...
from lib.detector.common.abstracts import Machinery
...
class VirtualBox(Machinery):
    ...
    def _initialize_check(self):
        ...
        @raise DetectorMachineError: if VBoxManage is not found.
        """
        # VirtualBox specific checks.
        if not self.options.virtualbox.path:
            raise DetectorCriticalError("VirtualBox VBoxManage path missing, "
                                      "please add it to the config file")
        if not os.path.exists(self.options.virtualbox.path):
            raise DetectorCriticalError("VirtualBox VBoxManage not found at "
                                      "specified path \"%s\"" %
