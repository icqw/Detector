...
from lib.detector.common.abstracts import Auxiliary
from lib.detector.common.constants import DETECTOR_ROOT, DETECTOR_GUEST_PORT
...
class Sniffer(Auxiliary):
    ...
    def start(self):
        ...
        file_path = os.path.join(DETECTOR_ROOT, "storage", "analyses",
        ...
        # Trying to save pcap with the same user which detector is running.
        try:
        ...
        pargs.extend([
            ...
            "dst", "port", str(DETECTOR_GUEST_PORT),
            ")", "and", "not", "(",
            "src", "host", self.machine.ip, "and",
            "src", "port", str(DETECTOR_GUEST_PORT),
            ")",
        ])
