...
try:
    from lib.detector.common.constants import DETECTOR_VERSION, DETECTOR_ROOT
    ...
    from lib.detector.common.logo import logo
    ...
    from lib.detector.core.startup import check_version, create_structure
    from lib.detector.core.startup import detector_clean, drop_privileges
    from lib.detector.core.startup import init_logging, init_modules
    from lib.detector.core.startup import init_tasks, init_yara, init_binaries
    from lib.detector.core.startup import init_rooter, init_routing

    import bson

    bson  # Pretend like it's actually being used (for static checkers.)
except (DetectorDependencyError, ImportError) as e:
    sys.exit("ERROR: Missing dependency: {0}".format(e))

log = logging.getLogger()

def detector_init(quiet=False, debug=False, artwork=False, test=False):
    """Detector initialization workflow.
...
if __name__ == "__main__":
    ...
    parser.add_argument("-v", "--version", action="version", version="You are running Detector Sandbox {0}".format(DETECTOR_VERSION))
    ...
    try:
        detector_init(quiet=args.quiet, debug=args.debug, artwork=args.artwork,
                    test=args.test)

        if not args.artwork and not args.test:
            detector_main(max_analysis_count=args.max_analysis_count)
    except DetectorCriticalError as e:
